...
\begin{questions}
	...
	\begin{parts}
		...
		\begin{solution}
			Using linearity of inner products, we get
			$$\braket{u+v}{u+v}$$
			$$ = \braket{u}{u+v} + \braket{v}{u+v}$$
			$$ = \braket{u}{u} + \braket{u}{v} + \braket{v}{u} + \braket{v}{v}$$
			Since $\ket{u}$ and $\ket{v}$ are orthogonal, we have
			$$ = \braket{u}{u} + 0 + 0 + \braket{v}{v} = \braket{u}{u} + \braket{v}{v} $$
		\end{solution}
		...
		\begin{solution}
			...
			Since $\ket{\mu} = \ket{u} - \mu \ket{v}$, $\bra{\mu} = \bra{u} - \mu^* \bra{v}$

			$\braket{\mu}{\mu}\geq 0$ for any $\mu$, so

			$$ \left(\bra{u} - \mu^* \bra{v} \right) \left( \ket{u} - \mu \ket{v} \right)$$
			$$ = \braket{u}{u} - \mu \braket{u}{v} - \mu^* \braket{v}{u} + \mu^* \mu \braket{v}{v} $$
			$$ = \braket{u}{u} - \mu \braket{u}{v} - \mu^* \braket{v}{u} + \abs{\mu}^2 \braket{v}{v} \geq 0$$
		\end{solution}
		...
		\begin{solution}
			From the last part,
			$$ \braket{u}{u} - \mu \braket{u}{v} - \mu^* \braket{v}{u} + \abs{\mu}^2 \braket{v}{v} \geq 0$$
			Using $\mu = \braket{v}{u} / \braket{v}{v}$,
			$$ \braket{u}{u} - \frac{\braket{v}{u}} {\braket{v}{v}} \braket{u}{v} - \frac{\braket{v}{u}^*} {\braket{v}{v}} \braket{v}{u} + \frac{\abs{\braket{v}{u}}^2} {\braket{v}{v}^2}\braket{v}{v} \geq 0$$
			Also, since $\braket{v}{u}\braket{v}{u}^*=\braket{v}{u}^*\braket{v}{u} = \abs{\braket{v}{u}}^2$,
			and $\braket{u}{v}=\braket{v}{u}^*$,
			$$ \braket{u}{u} - \frac{\abs{\braket{v}{u}}^2} {\braket{v}{v}} - \frac{\abs{\braket{v}{u}}^2} {\braket{v}{v}} + \frac{\abs{\braket{v}{u}}^2} {\braket{v}{v}} \geq 0$$
			$$ \braket{u}{u} - \frac{\abs{\braket{v}{u}}^2} {\braket{v}{v}}  \geq 0$$

			$$ \braket{u}{u} \geq \frac{\abs{\braket{v}{u}}^2} {\braket{v}{v}} $$
			$$ \braket{u}{u} \braket{v}{v} \geq \abs{\braket{v}{u}}^2 $$
		\end{solution}
	...
	\begin{parts}
		...
		\begin{solution}
			Given $\ket{f}$, $\bra{f} = \sum_{i=1}^{4} f_i^* \bra{i}$
			$$ \braket{f}{g} = \sum_{i=1}^{4} f_i^* \bra{i} \sum_{j=1}^{4} g_j \ket{j} $$
			$$ = \sum_{i=1}^{4} \sum_{j=1}^{4} f_i^* \bra{i}  g_j \ket{j} $$
			$$ = \sum_{i=1}^{4} \sum_{j=1}^{4} f_i^* g_j \braket{i}{j} $$
			$$ = \sum_{i=1}^{4} \sum_{j=1}^{4} f_i^* g_j \delta_{ij} $$
			$$ = \sum_{i=1}^{4} f_i^* g_i $$
		\end{solution}
		...
		\begin{solution}
			$f_3^* = f_4^*$, so $f_3^* + f_4^* = 2f_3^*$, and the same is valid for $g$, so
			$$\braket{f}{g} = f_1^* g_1 + f_2^* g_2 + f_3^* g_3 + f_4^* g_4$$
			$$ = f_1^* g_1 + f_2^* g_2 + 2f_3^* g_3 = \sum_{i=1}^{3} f_i^* g_i w_i$$ where $w_1=1$, $w_2=1$, and $w_3=2$.
		...
		\begin{solution}
			\begin{enumerate}
				\item $\braket{g}{f} = \sum_i g_i^* f_i w_i$,
				      so $\braket{g}{f}^* = \sum_i g_i f_i^* w_i^*$
				      But since $w_i$ is real, $w_i = w_i^*$ and
				      $$\braket{g}{f}^* = \sum_i g_i f_i^* w_i^* = \sum_i g_i f_i^* w_i = \braket{f}{g}$$

				\item $$\braket{f}{f} = \sum_i f_i^* f_i w_i = \sum_i \abs{f_i} w_i \geq 0$$
				      since $w_i$ is positive, this is a sum of all positive terms.


				\item If $f=0$, $\braket{f}{f} = 0$ is clear per 1(d). (multiplying zeros by weights wouldn't change anything)

				      Now suppose $\braket{f}{f} = 0$, thus
				      $$\braket{f}{f} = \sum_i \abs{f_i} w_i = 0$$
				      Since $w_i > 0$, $\abs{f_i} = 0$, for all $i$. Therefore $f=0$.

				\item $$\braket{f}{g+h} = \sum_i f_i^* (g_i+h_i) w_i  = \sum_i f_i^* g_i w_i+\sum_i f_i^* h_i w_i = \braket{f}{g} + \braket{f}{h}$$

				\item $$\braket{f}{\alpha g} = \sum_i f_i^* \alpha g_i w_i = \alpha \sum_i f_i^* g_i w_i = \alpha \braket{f}{g}$$
			\end{enumerate}
		\end{solution}

		\part Which property/properties in part (c) wouldn't be true if the weights $w$ were allowed to be complex?
		\begin{solution}
			1, since we use the fact that $w_i = w_i^*$.
		\end{solution}

		\part Which property/properties in part (c) wouldn't be true if the weights $w$ were allowed to be negative?
		\begin{solution}
			Considering non-zero negatives,
			2, since we use that $w_i>0$ to ensure it's a sum of positive terms.
		\end{solution}
		...
		\begin{solution}
			3. If a weight is zero, it can `kill' a non-zero term of a vector. A vector with a non-zero term isn't the null vector, but this way it could have a 0 product.
		\end{solution}
	...
	\begin{parts}
		...
		\begin{solution}
			$$\frac{\braket{j}{f}}{\braket{j}{j}} = \frac{\bra{j}\sum_{i} f_i \ket{i}}{n_j^2} = \sum_{i} f_i \frac{\braket{j}{i}}{n_j^2} = \sum_{i} f_i \frac{n_j^2 \delta{ji}}{n_j^2} = \sum_{i} f_i  \delta{ji} = f_j$$
		\end{solution}
		...
		\begin{solution}
			$$ \braket{f}{g} = \sum_{i} f_i^* \bra{i}   \sum_{j} g_j \ket{j} $$
			$$ = \sum_{i} \sum_{j} f_i^* \bra{i}  g_j \ket{j} $$
			$$ = \sum_{i} \sum_{j} f_i^* g_j \braket{i}{j} $$
			$$ = \sum_{i} \sum_{j} f_i^* g_j n_i^2 \delta_{ij} = \sum_{i} f_i^* g_i n_i^2$$
		\end{solution}
	\end{parts}
	...
	\begin{solution}
		...
		$$ D\ket{f} = \lim_{\epsilon\goesto0}\frac{T(0) - T(\epsilon)}{\epsilon} \ket{f}$$
		$$ = \lim_{\epsilon\goesto0} \frac{1}{\epsilon} \left[ \int\id{x} f(x) \ket{x} - \int\id{x} f(x-\epsilon) \ket{x}\right] $$
		$$ = \lim_{\epsilon\goesto0} \frac{1}{\epsilon} \left[ \int\id{x} (f(x)- f(x-\epsilon)) \ket{x} \right] $$
		As long as $f$ is dominated, the limit commutes with the integral:
		$$ =  \int\id{x} \left[ \lim_{\epsilon\goesto0} \frac{f(x)- f(x-\epsilon)}{\epsilon} \right] \ket{x} $$
		$$ =  \int\id{x} f'(x) \ket{x} $$
	...
	\begin{parts}
		...
		\begin{solution}
			It suffices to use the result from the previous question twice:
			$$ D\ket{f} = \int\id{x} f'(x) \ket{x} = \ket{f'}$$
			$$ L\ket{f} = D (D\ket{f}) = D (\ket{f'}) = \int\id{x} f''(x) \ket{x} $$
		\end{solution}
		...
		\begin{solution}
			...
			$$ L\ket{k} = \int \id{x}\; \frac{d}{dx}e^{i k x} \ket{x} $$
			$$ L\ket{k} = \int \id{x}\; (ik)^2 e^{i k x} \ket{x} $$
			$$ L\ket{k} = -k^2 \int \id{x}\;  e^{i k x} \ket{x} = -k^2 \ket{k}$$
		\end{solution}
		...
		\begin{solution}
			$$ \int \frac{\id{k}}{2\pi} \operator{k}{(-k^2)}{k} \ket{p}$$
			$$ = -k^2 \int \frac{\id{k}}{2\pi} \ket{k} \braket{k}{p}$$
			$$ = -k^2 \int \frac{\id{k}}{2\pi} \ket{k} 2 \pi \delta (p-k)$$
			$$ = -k^2 \int \id{k} \ket{k} \delta (p-k)$$
			$$ = -k^2\ket{p} = L\ket{p}$$
		\end{solution}
	\end{parts}
	...
	\begin{parts}
		...
		\begin{solution}
			...
			$$ x^2 + y^2 + z^2 = r^2 \left[\sin^2\theta \cos^2\phi + \sin^2 \theta \sin^2 \phi + \cos^2\theta\right] $$
			$$ x^2 + y^2 + z^2 = r^2 \left[\sin^2\theta (\cos^2\phi + \sin^2 \phi) + \cos^2\theta\right] $$
			$$ x^2 + y^2 + z^2 = r^2 \left[\sin^2\theta (1) + \cos^2\theta\right] $$
			$$ x^2 + y^2 + z^2 = r^2 \left[\sin^2\theta + \cos^2\theta\right] $$
			$$ x^2 + y^2 + z^2 = r^2 \left[ 1 \right] = r^2 $$

			$$ \frac{y}{x} = \frac{r\sin\theta\sin\phi}{r\sin\theta\cos\phi} = \frac{\sin\phi}{\cos\phi} = \tan \phi$$

			$$ \frac{z}{r} = \frac{r\cos\theta}{r} = \cos\theta $$
		\end{solution}
		...
		\end{align}
		\begin{solution}
			x) To obtain $x$, we get the vector $v$ of magnitude $r$, project it onto the plane $XY$, (which adds the factor of $\cos(\pi/2 - \theta) = \sin\theta$), then project it onto $X$, by adding $\cos \phi$. Then, $x = r\sin\theta\cos\phi$.

			y) To obtain $y$, we project $v$ onto $XY$, multiplying it by  $\cos(\pi/2 - \theta) = \sin\theta$) then project it onto $Y$, multiplying it by $\cos(\pi/2 - \phi) = \sin \phi$. $y = r \sin\theta\sin\phi$.

			z) Since the triangle with $\theta$ and $\phi$ are on a plane that passes through the axis $Z$, it suffices to project it onto $Z$ by multiplying by $\cos \theta$. $z=r\cos\theta$.
		\end{solution}
		...
		\begin{solution}
			\begin{equation}
				...
				\end{pmatrix}
			\end{equation}
			$$ = 0 + \sin\theta\sin\phi(-r\sin\theta) (-r\sin\theta\sin\phi) + \cos\theta r\cos\theta \cos\phi r\sin\theta\cos\phi $$
			$$ - (-r\sin\theta\sin\phi)r\cos\theta\sin\phi\cos\theta - r\sin\theta\cos\phi (-r\sin\theta) \sin\theta\cos\phi - 0...$$
			$$ = 0 + r^2 \sin^3\theta\sin^2\phi + r^2 \cos^2\theta\sin\theta\cos^2\phi + r^2 \cos^2 \theta \sin\theta \sin^2\phi +r^2 \sin^3\theta \cos^2\phi -0$$
			$$ = r^2 (\sin^3\theta\sin^2\phi + \cos^2\theta\sin\theta\cos^2\phi + \cos^2 \theta \sin\theta \sin^2\phi +\sin^3\theta \cos^2\phi)$$
			$$ = r^2 \sin\theta(\sin^2\theta\sin^2\phi + \cos^2\theta\cos^2\phi + \cos^2 \theta \sin^2\phi +\sin^2\theta \cos^2\phi)$$
			$$ = r^2 \sin\theta(\sin^2\theta\sin^2\phi + \sin^2\theta \cos^2\phi+ \cos^2\theta\cos^2\phi + \cos^2 \theta \sin^2\phi) $$
			$$ = r^2 \sin\theta(\sin^2\theta (\sin^2\phi \cos^2\phi)+ \cos^2\theta (\cos^2\phi + \sin^2\phi)) $$
		...
		\begin{solution} 
			$\id{u} = -\sin\theta \id\theta$ So
			$$ = \int_{0}^{\pi} (\id{\theta} \sin\theta) f(\cos\theta) $$
			$$ = \int_{0}^{\pi} (-\id{u}) f(u) $$
			$$ = -\int_{0}^{\pi} \id{u} f(u) $$
		\end{solution}
	\end{parts}
	...
	\clearpage
	\section*{As Always}
	\question How long did this problem set take you? 6h
